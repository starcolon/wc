...
var Scorer = function(scorerStr){
  ...
  var [ex,min] = when.match(/(pen|og)*(\d+\+*\d*)/).slice(1,3)
  ...
  addFixture: function(year,round,group,match,score,scorers){
    return F.loadTournament(year)
      ...
      .do(() => {
        ...
        var jobs = scorers.map((s) => {
          // TAOTODO: Determine which goal belongs to home, which belongs to away
          var sc = Scorer(s)
          sc.year = year;
          sc.round = round;
          sc.venue = ???;

          return __dbScorer.insert(sc)
        })
